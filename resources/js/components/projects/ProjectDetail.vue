        ...
        <div v-if="project.id > 0" class="mt-6">
            ...
            <div v-if="project.tasks.length > 0" class="my-4">
                <div class="flex justify-between">
                    ...
                    <button @click="addTask" class="bg-blue-500 rounded text-white px-3 py-2 hover:bg-blue-700">Add Task</button>
                </div>
                <div class="flex justify-center" v-show="showTaskForm">
                    <div>
                        <form @submit.prevent="handleTaskSubmit" class="shadow-md rounded px-8 pt-6 pb-8 mb-4">
                            ...
                            <div class="flex items-center">
                                <div class="w-1/3"></div>
                                <div class="w-2/3">
                                    <p v-show="taskErrMsg.length" class="mb-4 text-red-500 text-xs italic">{{ taskErrMsg }}</p>
                                </div>
                            </div>
                ...
                <task-item v-for="task in project.tasks" :key="task.id" :task="task" @delete-task="deleteTask" @edit-task="editTask"></task-item>
            </div>
        </div>
    </div>
</template>
